...
const count = db.prepare("SELECT COUNT(*) as count FROM software").get() as { count: number };
if (count.count === 0) {
  const insertSoftware = db.prepare(`
    INSERT INTO software (name, version, platforms, category, size, update_date, description, screenshots, popularity, download_url)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertMany = db.transaction((data) => {
    for (const item of data) {
      insertSoftware.run(item.name, item.version, item.platforms, item.category, item.size, item.update_date, item.description, item.screenshots, item.popularity, item.download_url);
    }
  });
  insertMany(seedData);
}
